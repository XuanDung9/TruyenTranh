...
namespace HamtruyenLibrary
{
    public class MainDb : IDisposable
    {
        ...
        public IEnumerable<T> FindRandom<T>(string[] fields,IMongoQuery query, int limitRow)
        {
            var _collection = GetCollection<T>();
            var max = _collection.Find(query).Count();
            Random ran = new Random(); int skipDocument = ran.Next((int)max);
            return _collection.Find(query).SetFields(fields).Skip(skipDocument).Take(limitRow);
        }
        ...
        {
            MongoCollection<T> t = GetCollection<T>();
            t.Save(_item);
        ...
        public IEnumerable<T> Find<T>(string[] fields, IMongoQuery query)
        {
            return GetCollection<T>().Find(query).SetFields(fields);
        }

        public IEnumerable<T> Find<T>(string[] fields, IMongoQuery query, IMongoSortBy sort)
        {
            return GetCollection<T>().Find(query).SetFields(fields).SetSortOrder(sort);
        }
        ...
        public IEnumerable<T> Find<T>(string[] fields, IMongoQuery query, IMongoSortBy sortby, int pageIndex, int pageSize)
        {
            return GetCollection<T>().Find(query).SetFields(fields).SetSkip((pageIndex - 1) * pageSize).SetLimit(pageSize).SetSortOrder(sortby);
        }
        ...
        {
            return GetCollection<T>().FindAll().SetSkip((pageIndex - 1) * pageSize).SetLimit(pageSize).SetSortOrder(sortBy);
        }
        ...
        {
            return GetCollection<T>().Find(query).SetSkip((pageIndex - 1) * pageSize).SetLimit(pageSize).SetSortOrder(sortBy);
        }
        ...
        public IEnumerable<T> Find<T>(string[] fields,IMongoQuery query, IMongoSortBy sortBy, int pageIndex, int pageSize, out long totalRows)
        {

            var _collection = GetCollection<T>();
            totalRows = _collection.Find(query).Count();
            return _collection.Find(query).SetFields(fields).SetSortOrder(sortBy).Skip((pageIndex - 1) * pageSize).Take(pageSize);//data.SetSkip((pageIndex - 1) * pageSize).SetLimit(pageSize);
        }
        ...
        public IEnumerable<T> All<T>(IMongoSortBy sortBy, int pageIndex, int pageSize, out long totalRows)
        {
            ...
            return data.FindAll().SetSkip((pageIndex - 1) * pageSize).SetLimit(pageSize).SetSortOrder(sortBy);

        }
        public IEnumerable<T> All<T>(IMongoSortBy sortBy, int pageIndex, int pageSize)
        {
            var data = GetCollection<T>();
            return data.FindAll().SetSkip((pageIndex - 1) * pageSize).SetLimit(pageSize).SetSortOrder(sortBy);
